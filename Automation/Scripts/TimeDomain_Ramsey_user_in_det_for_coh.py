...
if os.path.getsize('../Configuration_Files/Pulse_Calibrations/ramsey_time_limits.json') > 2:
...
with open('./ramsey_fits.json','r') as f:
    earlier_fits = json.load(f)
    f.close()

# t_min = 16 // 4  # ns
t_max = 5*earlier_fits['tau']*1000 // 4
dt = max(np.ceil(earlier_fits['tau']*10) // 4,16)  #
# time_limits = {}
# time_limits.update({str(q_no): {'t_min': t_min * 4, 't_max': t_max * 4, 'dt': dt * 4}})
# with open('./Pulse_Calibrations/ramsey_time_limits.json', 'w') as ram_times_file:
...
det = np.round(5/earlier_fits['tau'], 1)  # in MHz (give based on ramsey time)

print(f'detuning = {det} MHz')
print(f'dt = {dt}')
...
while res_handles.is_processing():
    ...
    if exception_flag == 0:
        ...
        if snr > 5:
            ...
            #
            # if flag_dt_high == 0:
            #     if (dt_us > 1 / (10 * guess_freq)) and (flag_dt_low == 0):
            #         # print(f'dt_us {dt_us}')
            #         # print(f'low flag = {flag_dt_low}')
            #         flag_dt_high = 1
            #         dt_p = copy.deepcopy(dt_us)
            #         dt_us1 = (1 / 20) * (1 / guess_freq)
            ...
            # if (flag_dt_high == 1 or flag_dt_low == 1 or flag_max_time_high == 1 or flag_max_time_low == 1):
