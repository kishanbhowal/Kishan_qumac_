...
elem_list = copy.copy(qe_list)
elem_list.extend(['stark_6'])
# for q_i in q_no_list:
#     elem_list.append(f'q12_{q_i}')
# elem_list = elem_list.extend(rr_list)
        ...
        with for_(depth, 1, depth <= max_circuit_depth, depth + delta_depth):
            ...
            with for_(n, 0, n < avgs, n + 1):
                ...
                for q_i in q_no_list:
                    ...
                    # cooldown(time=wait_init, qe=qe_list[q_index[f"{q_i}"]])
                ...
                for q_i in q_no_list:
                    # align(qe_list[q_index[f"{q_i}"]], rr_list[q_index[f"{q_i}"]])
...
if simulate:
    job = qmm.simulate(config, rb, SimulationConfig(int(15000)))
