...
a_list = np.linspace(0.05, 0.8, 10)
...
def HT_setAmp(cr_elem, a):
    def twoqubit_CR_ham_tomo_v2(cr_elem, control: int, target_pulse, echo: bool):
        '''
        ### Pulse protocol:
    ...
    with program() as CR_tomo:
        ...
        frame_rotation_2pi(cr_phase[cr_elem], cr_elem)
        with for_(n, 0, n < 300, n + 1):
            with for_(t, t_min, t < t_max, t + dt):
                ...
                wait(wait_init, qe_t)
                play_X180(qe_t, a=amp(0.25), t=t)
                wait(4, qe_t)
                measure_macro(qe_t, rr_t, out_t, Irabi, Qrabi, pi_12=pi_12)
                save(Irabi, I_rabi_st)
                save(Qrabi, Q_rabi_st)
                # play C0TI, get C0Z
                twoqubit_CR_ham_tomo_v2(cr_elem, 0, "I", Echo)

                # play C0TX/2, get C0Y
                twoqubit_CR_ham_tomo_v2(cr_elem, 0, "X90", Echo)

                # play C0T-Y/2, get C0X
                twoqubit_CR_ham_tomo_v2(cr_elem, 0, "mY90", Echo)

                # playing C1TI, get C1Z
                twoqubit_CR_ham_tomo_v2(cr_elem, 1, "I", Echo)

                # playing C1TX/2, get C1Y
                twoqubit_CR_ham_tomo_v2(cr_elem, 1, "X90", Echo)

                # playing C1T-Y/2, get C1X
                twoqubit_CR_ham_tomo_v2(cr_elem, 1, "mY90", Echo)
...
for a in a_list:
    ...
    fig.suptitle(f"Cross Resonance Tomography : Amplitude {a}", fontsize=15)
    ...
    if save_data is True:
        ...
        file_saver_qubit_(Itarget_data, file_name=__file__, master_folder=ExpName,
                          suffix=f"I_Control{c_no}_Target{t_no}_Amp_{a}", time_stamp=time_stamp)
        file_saver_qubit_(Qtarget_data, file_name=__file__, master_folder=ExpName,
                          suffix=f"Q_Control{c_no}_Target{t_no}_Amp_{a}", time_stamp=time_stamp)
        file_saver_qubit_(np.transpose([t_list_ns, Irabi, Qrabi]), file_name=__file__,
                          ...
                          suffix=f"Rabi_Control{c_no}_Target{t_no}_Amp_{a}", time_stamp=time_stamp)
        file_saver_qubit_(Icontrol_data, file_name=__file__,
                          master_folder=ExpName, header_string="Time (in ns), Magnitude, I, Q",
                          suffix=f"Icontrol_Control{c_no}_Target{t_no}_Amp_{a}", time_stamp=time_stamp)
        file_saver_qubit_(Qcontrol_data, file_name=__file__,
                          master_folder=ExpName, header_string="Time (in ns), Magnitude, I, Q",
                          suffix=f"Qcontrol_Control{c_no}_Target{t_no}_Amp_{a}", time_stamp=time_stamp)
