...
qe = qe_t
qe_t_12 = f"q12_{q_no_t}"
qe_c_12 = f"q12_{q_no_c}"
qe_c = f"q{q_no_c}"
rr_c = f"rr{q_no_c}"
...
with program() as ramsey:
    ...
    with for_(n, 0, n < 4000, n + 1):
        ...
        #     reset_frame(rr_t)
            ...
            play_X180(qe_c_12, a=amp(0.1), t=t)
            ...
            measure_macro(qe_t, rr_c, out_c, I0c, Q0c, pi_12=pi_12)
            save(I0, I0_st)
            save(Q0, Q0_st)
            save(I0c, I0c_st)
            save(Q0c, Q0c_st)
...
fig.suptitle(f"Normal Rabi on {qe_t}  with pi init on {qe_c}  with readout on both control and target")
lines = []
tc = ["Rabi", "Control"]
q_no = [q_no_t, q_no_c]
for i in range(2):
    lines.append(ax[i].plot(4e-3 * t_list, [0]*len(t_list), marker=".", label="0")[0])  # Returns a tuple of line objects, thus the comma
...
while res_handles.is_processing():
    ...
    for i in range(1):
        ...
        ax[i].axhline(t1, color ='red')
        ax[i].axhline(t0, color ='green')
        ax[i+1].axhline(c1, color = 'red')
        ax[i+1].axhline(c0, color = 'green')
