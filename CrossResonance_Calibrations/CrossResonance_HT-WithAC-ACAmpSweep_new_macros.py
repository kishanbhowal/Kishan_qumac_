...
def HT_setACAmp(cr_elem, cr_ac_elem, a_ac):
    def twoqubit_CR_ham_tomo_withAC(control: int, target_pulse, cr_elem, cr_ac_elem, a_ac, echo: bool, simulate=False):
        '''
        ### Pulse protocol:
...
a_ac_list = np.linspace(-0.1, 0.1, 3)
for a_ac in a_ac_list:
    ...
    if save_data is True:
        file_saver_qubit_(Itarget_data, file_name=__file__, master_folder=ExpName,
                          suffix=f"I_Control{c_no}_Target{t_no}_ACAmp_{a_ac}_ac_phase_{cr_phase[cr_ac_elem]}", time_stamp=time_stamp)
        file_saver_qubit_(Qtarget_data, file_name=__file__, master_folder=ExpName,
                          suffix=f"Q_Control{c_no}_Target{t_no}_ACAmp_{a_ac}_ac_phase_{cr_phase[cr_ac_elem]}", time_stamp=time_stamp)
        file_saver_qubit_(np.transpose([t_list_ns, Irabi, Qrabi]), file_name=__file__,
                          ...
                          suffix=f"Rabi_Control{c_no}_Target{t_no}_ACAmp_{a_ac}_ac_phase_{cr_phase[cr_ac_elem]}", time_stamp=time_stamp)
        file_saver_qubit_(Icontrol_data, file_name=__file__,
                          master_folder=ExpName, header_string="Time (in ns), Magnitude, I, Q",
                          suffix=f"Icontrol_Control{c_no}_Target{t_no}_ACAmp_{a_ac}_ac_phase_{cr_phase[cr_ac_elem]}", time_stamp=time_stamp)
        file_saver_qubit_(Qcontrol_data, file_name=__file__,
                          master_folder=ExpName, header_string="Time (in ns), Magnitude, I, Q",
                          suffix=f"Qcontrol_Control{c_no}_Target{t_no}_ACAmp_{a_ac}_ac_phase_{cr_phase[cr_ac_elem]}", time_stamp=time_stamp)
