...
a_cr = 1.5*amp_scale[str(c_no)]["X180"]
a_cr = cr_amp[cr_elem]


def HT_setPhase(cr_elem, p):
    def twoqubit_CR_ham_tomo_v2(cr_elem, control: int, target_pulse, a_cr=0.3, echo: bool = False):
        '''
        ### Pulse protocol:
    ...
    with program() as CR_tomo:
        ...
        with for_(n, 0, n < 200, n + 1):
            with for_(t, t_min, t < t_max, t + dt):
                ...
                # play C0TI, get C0Z
                twoqubit_CR_ham_tomo_v2(cr_elem, 0, "I", a_cr, Echo)

                # play C0TX/2, get C0Y
                twoqubit_CR_ham_tomo_v2(cr_elem, 0, "X90", a_cr, Echo)

                # play C0T-Y/2, get C0X
                twoqubit_CR_ham_tomo_v2(cr_elem, 0, "mY90", a_cr, Echo)

                # playing C1TI, get C1Z
                twoqubit_CR_ham_tomo_v2(cr_elem, 1, "I", a_cr, Echo)

                # playing C1TX/2, get C1Y
                twoqubit_CR_ham_tomo_v2(cr_elem, 1, "X90", a_cr, Echo)

                # playing C1T-Y/2, get C1X
                twoqubit_CR_ham_tomo_v2(cr_elem, 1, "mY90", a_cr, Echo)
