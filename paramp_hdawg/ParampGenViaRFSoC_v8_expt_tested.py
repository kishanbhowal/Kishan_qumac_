...
f_sep = [[800e6,800e6,800e6,800e6],[800e6,800e6,800e6,800e6]] # Frequency Separation of dual tones

# AMPLITUDE PLAN
Amp1_offset = [[0,0,0,0],[0,0,0,0]] # DC Offset
Amp1 = [[0.5,0.5,0.5,0.55],[0.75,0.45,0.4,0.55]]
tot_pwr = [[0.6,0.5,0.6,0.6],[0.6,0.6,0.6,0.6]]


# DAC CURRENT PLAN
DAC_currents = [[14.75,14.5,13.175,15],[10,20,14,20]]       # currents in mA
...
cavity_freqs = [[7.3e3, 7.375748e3,7.375748e3,7.3e3],[7.413061e3,7.3e3,7.3e3,7.3e3]]
...
for i in range(len(Blocks)):
    for j in range(len(Blocks[i])):
        [wave_data[i][j],_,_] = gen_pulses(fs_IF, N, f_c[i][j], f_sep[i][j], Amp1_offset[i][j], Amp1[i][j],tot_pwr[i][j])
...
# [data,_,_] = gen_pulses(fs_IF, N, 0, 800e6, 0, 0.5,0.5)

if pump:
    [data,_,_] = gen_pulses(fs_IF, N, 0, 1600e6, 0, 0.5,tot_pwr1=1)
    DAC_prog(board, 1, Tile, DAC_no, F_sample, 29, Cavity_freqs[Tile][DAC_no], data)
else:
    [data,_,_] = gen_pulses(fs_IF, N, 0, 800e6, 0, 0.5,0.0)
    DAC_prog(board, 1, Tile, DAC_no, F_sample, 10, Cavity_freqs[Tile][DAC_no], data)
    

#%% POST CALIBRATION STATUS CHECK / UPDATE
change_current(29,DAC_no,Tile)
#%% PROGRAM DAC
...
#         print(i)
#         print(j)
#         DAC_prog(board, Type, Tiles[i][j], Blocks[i][j], F_sample, DAC_currents[i][j], Cavity_freqs[i][j], wave_data[i][j])
    
#%% POST CALIBRATION STATUS CHECK / UPDATE

# change_current(206,1,1)
# #%%    data = 0
# [data,_,_] = gen_pulses(fs_IF, N, 0, 800e6, 0, 0.5,0.5)

# #%%

# DAC_prog(board, 1, 1, 1, F_sample, 22, Cavity_freqs[1][1], data)
# # %%
