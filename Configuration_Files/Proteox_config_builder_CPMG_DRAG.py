from Helper_Functions.Proteox_helper_functionsv2_CPMG_DRAG import *
        ...
        if 'grft' in pul:
            ...
            config["waveforms"][f"{pul}_I_wf"] = {"type": "arbitrary",
                                                      "samples": grft_arr_gen(
                                                          (pi_len_ns[f"{q_no}"], pi_rise_grft_ns), 1)}
        ################NEW#############
        ...
        if "180" in pul:
            if "X" in pul:
                ...
                if 'grft' in pul:
                    ...
                    config["waveforms"][f"{pul}_I_wf"] = {"type": "arbitrary",
                                                          "samples": grft_arr_gen(
                                                              (pi_len_ns[f"{q_no}"], pi_rise_grft_ns),
...
def config_add_drag_grft(config, q_no, q_anh, pi_len_ns, piby2_len_ns, pi_rise_grft_ns, amp_scale, drag_dict):
    ...
    config['elements'][f"q{q_no}"]["operations"].update({"d_X180": f"q{q_no}_d_X180",
                                                         "d_X180C1": f"q{q_no}_X180C1",
                                                         "d_X180C2": f"q{q_no}_X180C2",
                                                         "d_X180C3": f"q{q_no}_X180C3",
                                                         "d_X180C4": f"q{q_no}_X180C4",
                                                         "d_X180C5": f"q{q_no}_X180C5",
                                                         "d_X180C6": f"q{q_no}_X180C6",
                                                         "d_X90": f"q{q_no}_d_X90",
                                                         "d_X90C1": f"q{q_no}_X90C1",
                                                         "d_X90C2": f"q{q_no}_X90C2",
                                                         "d_X90C3": f"q{q_no}_X90C3",
        ...
        if "d" in pul:
            ...
            if "X" in pul:
                ...
                if "m" in pul:
                    ...
                    config["waveforms"][f"{pul}_I_wf"] = {"type": "arbitrary",
                                                          "samples": drag_grft_CPMG_pulse_waveforms(
                                                              amplitude=-amp_scale[f"{q_no}"][f"{pul_name}"],
                                                              length=pi_len_ns[f"{q_no}"], rise=pi_rise_grft_ns,
                                                              n0=2,
                                                              anharmonicity=q_anh[f"{q_no}"],
                ...
                if "X" in pul:
                    ...
                    if "m" in pul:
                        config["waveforms"][f"{pul}_I_wf"] = {"type": "arbitrary",
                                                              "samples": drag_grft_pulse_waveforms(
                                                                  amplitude=-amp_scale[f"{q_no}"][f"{pul_name}"],
                                                                  length=piby2_len_ns[f"{q_no}"], rise=pi_rise_grft_ns,
                                                                  anharmonicity=q_anh[f"{q_no}"],
                                                                  alpha=drag_dict[f"{q_no}"]["alpha"],
                                                                  detuning=drag_dict[f"{q_no}"]["det"])[0]}
