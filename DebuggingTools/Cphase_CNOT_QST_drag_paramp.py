...
from Aneesh_configuration_octave_v2 import *
from qualang_tools.results import progress_counter, fetching_tool
import numpy as np
import matplotlib.pyplot as plt
from macros_drag_paramp import *
...
if qe_t == 'q1' and qe_c == 'q3':
    ro_len = ro_len_clk[str(t_no)]
else:
    ro_len = ro_len_clk[str(c_no)]

# t_fgge = 196//4
# # raman_IF = 25.10 * u.MHz - 3.25 * u.MHz #best_one
# raman_IF = 25.10 * u.MHz - 2.85 * u.MHz #FOR Q3 as control
# # raman_IF = 25.10 * u.MHz - 1.60 * u.MHz #FOR Q2 as control
# a_ram = 0.80

t_fgge = 524//4 #pair Q3-Q1
raman_IF = 128.07 * u.MHz - 0.64 * u.MHz
a_ram = 1.0
...
paramp = True
def pump_tone(qe,pump_qe,rr,pump_amp):
    align(qe, pump_qe, rr)
    wait(wait_rr,rr)
    play("const"*amp(pump_amp), pump_qe, duration=ro_len+wait_rr)

pump_no = 4
pump_qe= f"rr{pump_no}"
pump_amp = ro_amp[str(pump_no)]
...
def two_qubit_tomo_pulse(tomo_pulse1,tomo_pulse2):
    ...
    align()
    wait(rep_rate_clk)
    prepare_pulse_state(qe_c, target_rot1)
    align(qe_c,qe_t)
    prepare_pulse_state(qe_t, target_rot2)
    wait(wait_t,qe_t)
    wait(wait_t,qe_c)

    # CZ_Cphase_macro(cz_elem,qe_c,qe_t,a_ram,t_fgge)
    # CZ_CNOT_macro(cz_elem,qe_c,qe_t,a_ram,t_fgge)
    CZ_SWAP_macro(cz_elem,qe_c,qe_t,a_ram,t_fgge)

    align()
    wait(wait_t, qe_c)
    wait(wait_t, qe_t)
    tomo_pulse_state(qe_c, tomo_pulse1)
    tomo_pulse_state(qe_t, tomo_pulse2)
    align(qe_c,qe_t,rr_c,rr_t)

    if qe_t == 'q1' and qe_c == 'q3':
        if paramp:
                pump_tone(qe_t,pump_qe,rr_t,pump_amp)

    else:
        if paramp:
                pump_tone(qe_c,pump_qe,rr_c,pump_amp)
    measure_macro(qe_c, rr_c, out_c, I1, Q1, pi_12 = False)
    measure_macro(qe_t, rr_t, out_t, I2, Q2, pi_12 = False)
    save(I1, I1_st)
    save(Q1, Q1_st)
    save(I2, I2_st)
    save(Q2, Q2_st)
...
with program() as two_qubit_tomo:
    ...
    with for_(n, 0, n < n_avgs, n + 1):
        ...
        if qe_t == 'q1' and qe_c == 'q3':
            ...
            if paramp:
                pump_tone(qe_c,pump_qe,rr_c,pump_amp)
        measure_macro(qe_c, rr_c, out_c, Ig1, Qg1, pi_12 = False)
        save(Ig1, Ig1_st)
        save(Qg1, Qg1_st)
        measure_macro(qe_t, rr_t, out_t, Ig2, Qg2, pi_12= False)
        save(Ig2, Ig2_st)
        save(Qg2, Qg2_st)
        ...
        if qe_t == 'q1' and qe_c == 'q3':
            ...
            if paramp:
                pump_tone(qe_c,pump_qe,rr_c,pump_amp)
        measure_macro(qe_c, rr_c,out_c, Ie1, Qe1, pi_12 = False)
        save(Ie1, Ie1_st)
        save(Qe1, Qe1_st)
        measure_macro(qe_t, rr_t, out_t, Ie2, Qe2, pi_12 = False)
        save(Ie2, Ie2_st)
        save(Qe2, Qe2_st)

        align(qe_c,rr_c,qe_t,rr_t)
        #Starting_tomography
        two_qubit_tomo_pulse("I", "I")  #Z measurement
        two_qubit_tomo_pulse("d_X90", "d_X90") #Y measurement
        two_qubit_tomo_pulse("d_mY90", "d_mY90") #X measurement
...
if save_data:
    file_saver_([Ig1, Qg1, Ig2, Qg2, Ie1, Qe1, Ie2, Qe2, I1[0], Q1[0], I1[1], Q1[1],I1[2], Q1[2], I2[0], Q2[0], I2[1], Q2[1],I2[2], Q2[2]], suffix= f"data_q{q1}_{target_rot1}_q{q2}_{target_rot2}", file_name=__file__,
