...
f_min = 100 * u.MHz#141.52 * u.MHz#38.5 * u.MHz#132 * u.MHz #129.13 12.171
f_max = 200 * u.MHz#141.58 * u.MHz#132.5 * u.MHz # -91.4
...
with program() as qubit_spec:
    ...
    with for_(n, 0, n < 10000, n + 1):
        with for_(f, f_min, f < f_max, f + df):
            reset_frame(qe_t)         # added for testing remove asap
            reset_frame(qe_c)
            # play("const" * amp(0), q_stark, duration=wait_time+pulse_duration)
            # play("const" * amp(1), q_stark)
            play(ramp(0.0025 * 0.4), 'stark_6', duration=80)
            wait(wait_time)


            #
            update_frequency(qe_c, f)
            play("const"*amp(q_amp), qe_c, duration=pulse_duration)
            align(q_stark, qe_c)
            # play("const"*amp(0.0), q_stark)
