...
for calib in ["180", "90"]:
    ...
    if calib == "180":
        ...
        operation = pulse + "X180"
        if calib_Y:
            operation = pulse + "Y180"
    if calib == "90":
    ...
    print('#########################')
    print(f"Rabi frequency {calib} axis= {np.round(pars[1], 2)} per amp")
    if calib_Y:
        print(f"{calib} pulse amplitude = {pi_amp * 2 * amp_scale[str(q_no)]['Y'+calib]}")
    else:
        print(f"{calib} pulse amplitude = {pi_amp * 2 * amp_scale[str(q_no)]['X'+calib]}")
    ...
    plt.ylabel("Rabi Amplitude")
    if calib_Y:
        plt.title(f"Power Rabi : {calib} amp = {pi_amp * 2 * amp_scale[str(q_no)]['Y180']} ; N = {n_pulses}")
    else:
        plt.title(f"Power Rabi : {calib} amp = {pi_amp * 2 * amp_scale[str(q_no)]['X180']} ; N = {n_pulses}")
    plt.axvline(pi_amp)
    plt.legend()
    plt.ticklabel_format(axis='y', style='sci', scilimits=(0, 0))
    plt.grid()
