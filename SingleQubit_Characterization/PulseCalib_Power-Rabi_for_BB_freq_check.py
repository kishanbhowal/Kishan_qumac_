...
for calib in ["Pi"]:
    ...
    with program() as power_rabi:
        ...
        with stream_processing():
            I_st.buffer(len(x_n)).average().save('I')
            Q_st.buffer(len(x_n)).average().save('Q')
    ...
    ####################
    if simulate:
        job = qmm.simulate(config, power_rabi, SimulationConfig(int(100000)))
        # get DAC and digital samples
        samples = job.get_simulated_samples()
        # plot all ports:
        con = dac_mapping[qe][0]
        samples = job.get_simulated_samples()
        samples.con3.plot()
        plt.legend("")
    ...
    #
    #
    # pars, cov = curve_fit(f=rabi_fit, xdata=np.arange(1, n_pulses, 1), ydata=I, p0=[5e-4, c * N * amp_guess, 300, 0, 1e-5],
    #                       bounds=(-np.inf, np.inf), maxfev=5000)

    #
    #
    # amp_arr = np.linspace(1, n_pulses, 2000)
